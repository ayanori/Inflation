function [u,v,w] = okada(fault,xi,yi,xf,yf,zt,zb,U,delta,mu,nu,x,y)
%%
%% This routine was modefied by Y Cheng in Feb 2021
%% Original remarks by USGS:
...
if strcmpi(fault,'strike')==1
    ...
    [ux1,uy1,uz1,~,~,~,~,~,~] = ok8525(xxp,p,q,mu,lambda,-U,delta);
    [ux2,uy2,uz2,~,~,~,~,~,~] = ok8525(xxp,p-W,q,mu,lambda,-U,delta);
    [ux3,uy3,uz3,~,~,~,~,~,~] = ok8525(xxp-L,p,q,mu,lambda,-U,delta);
    [ux4,uy4,uz4,~,~,~,~,~,~] = ok8525(xxp-L,p-W,q,mu,lambda,-U,delta);
elseif strcmpi(fault,'dip')==1
    % dip slip fault displacement and displacement gradient matrix. Okada (1985), equation (26)
    [ux1,uy1,uz1,~,~,~,~,~,~] = ok8526(xxp,p,q,mu,lambda,U,delta);
    [ux2,uy2,uz2,~,~,~,~,~,~] = ok8526(xxp,p-W,q,mu,lambda,U,delta);
    [ux3,uy3,uz3,~,~,~,~,~,~] = ok8526(xxp-L,p,q,mu,lambda,U,delta);
    [ux4,uy4,uz4,~,~,~,~,~,~] = ok8526(xxp-L,p-W,q,mu,lambda,U,delta);
else 
    % tensile fault displacement and displacement gradient matrix. Okada (1985), equation (27)
    [ux1,uy1,uz1,~,~,~,~,~,~] = ok8527(xxp,p,q,mu,lambda,U,delta);
    [ux2,uy2,uz2,~,~,~,~,~,~] = ok8527(xxp,p-W,q,mu,lambda,U,delta);
    [ux3,uy3,uz3,~,~,~,~,~,~] = ok8527(xxp-L,p,q,mu,lambda,U,delta);
    [ux4,uy4,uz4,~,~,~,~,~,~] = ok8527(xxp-L,p-W,q,mu,lambda,U,delta);    
end
